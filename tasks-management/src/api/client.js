...
export const categoriesApi = {
  ...
  create: (name, department_id) => USE_MOCK
    ? mock.createCategory(name, { department_id })
    : request('/categories', { method: 'POST', body: JSON.stringify({ name, department_id }) }),
  update: (oldName, newName) => USE_MOCK
    ? mock.updateCategory(oldName, newName)
    : request('/categories', { method: 'PUT', body: JSON.stringify({ oldName, newName }) }),
  remove: (name) => USE_MOCK
...
export const tasksApi = {
  ...
  progress: (id, data) => USE_MOCK
    ? mock.taskProgress(id, data)
    : request(`/tasks/${id}/progress`, { method: 'PATCH', body: JSON.stringify(data) }),

  delegate: (id, data) => USE_MOCK
    ? mock.delegateTask(id, data)
    : request(`/tasks/${id}/delegate`, { method: 'POST', body: JSON.stringify(data) }),

  // User submits completed work for manager approval
  submit: (id, data) => USE_MOCK
    ? mock.submitTask(id, data)
    : request(`/tasks/${id}/submit`, { method: 'POST', body: JSON.stringify(data) }),

  // Manager/Admin approves task completion
  approve: (id, data) => USE_MOCK
    ? mock.approveTask(id, data)
    : request(`/tasks/${id}/approve`, { method: 'POST', body: JSON.stringify(data) }),

  // Creator/Delegator responds to a Blocker task (accept | in_progress)
  blockerResponse: (id, data) => request(`/tasks/${id}/blocker-response`, { method: 'POST', body: JSON.stringify(data) }),

  // Admin/Manager escalates overdue task to MD
  escalate: (id, data) => request(`/tasks/${id}/escalate`, { method: 'POST', body: JSON.stringify(data) }),
...
export const reportsApi = {
  weekly: (params = {}) => USE_MOCK
    ? mock.weeklyReport(params)
    : request('/reports/weekly?' + new URLSearchParams(
...
export const kpisApi = {
  ...
  addValue: (id, data) => USE_MOCK
    ? mock.kpiAddValue(id, data)
    : request(`/kpis/${id}/update`, { method: 'PATCH', body: JSON.stringify(data) }),
}
...
export const dailyLogsApi = {
  ...
  list: (params = {}) => USE_MOCK
    ...
    : request('/daily-logs?' + new URLSearchParams(
        Object.fromEntries(Object.entries(params).filter(([, v]) => v !== undefined && v !== ''))
      )),
  ...
  update: (id, data) => USE_MOCK
    ...
    : request(`/daily-logs/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
